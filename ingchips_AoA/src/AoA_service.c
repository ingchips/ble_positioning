...
int8_t  g_devNum = 0;
int16_t g_iqReportStrLen = AoA_IQ_STR_LEN_FIRST_CP + AoA_IQ_STR_LEN_SECOND + 1;
...
#if AoA_MANAUL_SETUP_ARRAY
static void AoA_CheckNewDevice(void)
{
    if (g_devNum == AoA_DEVICE_NUM_MAX ||
        !memcmp(g_deviceList[0].deviceAddr, g_deviceList[g_devNum].iq_str_buffer_tmp, AoA_DEVICE_ADDR_LEN) ||
        !memcmp(g_deviceList[1].deviceAddr, g_deviceList[g_devNum].iq_str_buffer_tmp, AoA_DEVICE_ADDR_LEN) ||
        !memcmp(g_deviceList[2].deviceAddr, g_deviceList[g_devNum].iq_str_buffer_tmp, AoA_DEVICE_ADDR_LEN)) {
        printf("AoA_CheckNewDevice: bad new device address!\n");
        return;
    }

    memcpy(g_deviceList[g_devNum].deviceAddr, g_deviceList[g_devNum].iq_str_buffer_tmp, AoA_DEVICE_ADDR_LEN);
    g_devNum++;
    g_checkNewFlg = 0;
