...
static void output_notification_handler(uint8_t packet_type, uint16_t channel, const uint8_t *packet, uint16_t size)
{
    const gatt_event_value_packet_t *value;
    uint16_t value_size;
    int8_t iq_str_buffer[AoA_IQ_STR_BUFFER_LEN_TMP];
    switch (packet[0])
    {
    case GATT_EVENT_NOTIFICATION:
        ...
        if (value_size) {
            int i;
            strcpy(iq_str_buffer, value->value);
            // iprintf("NOTIFACATION of %d:\n", value->handle);
            // print_hex_table(value->value, value_size, print_fun);
#if AoA_MANAUL_SETUP_ARRAY
            if (strlen(iq_str_buffer) == AoA_IQ_STR_LEN_FIRST) {
                for (i = 0; i < g_devNum; i++) {
                    if (!memcmp(g_deviceList[i].deviceAddr, iq_str_buffer, AoA_DEVICE_ADDR_LEN)) {
                        memcpy(g_deviceList[i].iq_str_buffer_tmp, iq_str_buffer, AoA_IQ_STR_LEN_FIRST);
                        break;
                    }
                }
                if (g_checkNewFlg && (g_devNum != AoA_DEVICE_NUM_MAX) && (i == g_devNum)) {
                    memcpy(g_deviceList[i].iq_str_buffer_tmp, iq_str_buffer, AoA_IQ_STR_LEN_FIRST);
                }
            } else if (strlen(iq_str_buffer) == AoA_IQ_STR_LEN_SECOND) {
                for (i = 0; i < g_devNum; i++) {
                    if (!memcmp(g_deviceList[i].iq_str_buffer_tmp + AoA_IQ_STR_LEN_FIRST_CP, iq_str_buffer, AoA_IQ_STR_LEN_CHECK)) {
                        memcpy(g_deviceList[i].iq_str_buffer, g_deviceList[i].iq_str_buffer_tmp, AoA_IQ_STR_LEN_FIRST_CP);
                        memcpy(g_deviceList[i].iq_str_buffer + AoA_IQ_STR_LEN_FIRST_CP, iq_str_buffer, AoA_IQ_STR_LEN_SECOND + 1);
                        strcat(g_deviceList[i].iq_str_buffer, "\n");
                        break;
                    }
                }
            }
