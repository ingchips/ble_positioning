...
void recv_pro_connless_iq_report(const le_meta_pro_connless_iq_report_t *report)
{
    ...
    if (notify_enable) {
#ifdef AOA_REPORT
		char iq_str_tmp_buff[120];
		sprintf(iq_str_tmp_buff, "%X:%X:%X:%X:%X:%X", rand_addr[0], rand_addr[1], rand_addr[2], 
			rand_addr[3], rand_addr[4], rand_addr[5]);
		memcpy(iq_str_tmp_buff + 17, iq_str_buffer, 100);
		iq_str_tmp_buff[117] = '\0';
        att_server_notify(handle_send, HANDLE_GENERIC_OUTPUT, (uint8_t *)iq_str_tmp_buff, 118);
        att_server_notify(handle_send, HANDLE_GENERIC_OUTPUT, (uint8_t *)(iq_str_buffer + 90), strlen(iq_str_buffer) - 89);
#else
		att_server_notify(handle_send, HANDLE_GENERIC_OUTPUT, (uint8_t *)iq_str_buffer, strlen(iq_str_buffer) + 1);
#endif
    } else {
